...
struct MipmapLevel CompressAndNOTGenerateMipmaps(const byte *data, int width, int height) {
    ...
    byte *dstData = (byte*)malloc(bw * bh * 8);
    CompressRGTCFromRGBA8_Gen(srcData, levw, levh, 4 * levw, dstData);

    result.data = dstData;
    result.width = levw;
    result.height = levh;
    result.sizeInBytes = bw * bh * 8;
...
void WriteDDS(const char *filename, const struct MipmapLevel levels) {
    ...
    memcpy(&header.ddspf.dwFourCC, "ATI1", 4);
...
int main() {
    ...
    char *filename = "woodcontainer1"; //pinkpurplecircle multicolor1 woodcontainer1 woodfloor1
    char *path1 = "test_pics\\";
    char *readpath2 = ".tga";
    char *writepath2 = "_RGTC_RED_encoded.dds";
